...
form.addEventListener("submit", function (e) {
    // submit trimite automat mesajul catre server, de asta e

    e.preventDefault();
// valoare  introdusa in value este string, de asta parseInt
    const value = parseInt(amount.value);
    // un numar random in functie de lungimea array
    const random = Math.floor(Math.random() * text.length);

    if (isNaN(value) || value <= 0 || value > 9) {
        result.innerHTML = `<p class="result">${text[random]}</p>`;
    } else {
        // slice 2 parametri: prima si pana unde
