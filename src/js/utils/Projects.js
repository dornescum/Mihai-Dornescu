...
<b>Ops</b> — <code>docker-compose.yml</code> for local Postgres + pgAdmin; nginx proxies to <code>:8080</code> and serves <code>/public/uploads/</code> directly. Stripe webhook handler and pgvector embedding pipeline are the next milestones.`
  ...
  per-day macro/micronutrient totals (protein, carbs, vitamin C, etc.) computed from logged meals.`,
          tools: "<span style='color:#00ADD8;font-weight:bold'>Go</span>, Chi, MySQL, golang-migrate, JWT, HTMX, Swagger, Docker",
		  ...
		  tagDB: "Mysql",
          tagTutorial: false,
          desc: `<span style='color:#00ADD8;font-weight:bold'>Go</span>, Go/Chi nutrition tracker with JWT auth, MySQL schema, USDA ingredient data, custom diet plans and nutrient calculations`,
  ...
  step) and a <b>MySQL</b> schema managed with <b>golang-migrate</b>.<br><br><b>Database</b> — 32 migrations, split into schema and seed pairs: <code>roles</code>, <code>users</code>, <code>refresh_tokens</code>,
  <code>categories</code> (5 food groups: Fruits/Vegetables/Grains/Protein/Dairy), <code>ingredients</code> + <code>item_quantities</code>, <code>nutrient_types</code> + <code>ingredient_nutrients</code>,
  ...
  migrations.<br><br><b>Nutrition engine</b> — each ingredient carries 20 seeded nutrient types (Calories, Protein, Carbohydrates, Sugar, Fiber, Fat, Saturated Fat, Cholesterol, Sodium, Potassium, Calcium, Iron,
  Magnesium, Zinc, Phosphorus, Vitamins A/C/D/E, B12) stored per 100g. Logging a meal at <code>/diet</code> scales every nutrient to the logged weight in Go (<code>ScaledNutrients</code>), and daily totals are
  summed across all logged entries for that date.<br><br><b>Custom diets</b> — <code>/diet-plans</code> lets a user build a multi-day plan (day-by-day ingredient assignment via
  <code>/diet-plans/{id}/days/{day_number}</code>), while <code>/diets</code> lets them tag day-to-day logs under a named, ongoing diet (<code>user_diets</code>) to track adherence over time.<br><br><b>Auth &amp;
  routing split</b> — JWT access/refresh tokens (refresh stored server-side, rotated via <code>/auth/refresh</code>). Three route groups behind separate middleware: public (home, categories, ingredient browser,
